...
import { ScoreItem } from "src/core/model/impl/score-item";
...
export class LiveScoresService {
  ...
  formatFeedData(feed): { scores: IScoreItem[], competition: ICompetition } {
    if (feed) {
      const competition = feed.c;
      const games = feed.g;
      const scores = games
        .sort((gameOne, gameTwo) => (gameOne.c > gameTwo.c ? gameOne : gameTwo))
        .map(feedItem => new ScoreItem(feedItem, competition));
